#!/bin/bash
# Script to invoke the SecurityHub Compliance Analyzer Lambda for a test email to the new address
...
aws lambda invoke \
  ...
  test_new_email_response.json

echo "Response saved to test_new_email_response.json"

# Print the response
echo "Response:"
cat test_new_email_response.json