...
def get_findings(hours, framework_id=None):
    ...
    try:
        ...
        if framework_id:
            ...
            # Get framework ARN
            framework_arn = next(
                (f["arn"] for f in frameworks if f["id"] == framework_id), None
            )
            if framework_arn:
        ...
        for finding in findings:
            ...
            for framework in frameworks:
                ...
                if (
                    "ProductFields" in finding
                    and "StandardsArn" in finding["ProductFields"]
                ):
...
def get_nist_control_status(findings=None):
    ...
    try:
        ...
        try:
            ...
            logger.info(
                f"Retrieved data for {len(control_status)} NIST 800-53 controls"
            )
        except Exception as e:
...
def cli_handler():
    ...
    parser.add_argument(
        "--hours", type=int, default=24, help="Hours of findings to analyze"
    )
    parser.add_argument(
        "--framework", type=str, default="SOC2", help="Compliance framework"
    )
    parser.add_argument(
...
def lambda_handler(event, context):
    ...
    try:
        ...
        return {"statusCode": 500, "body": {"message": f"Error: {str(e)}"}}
